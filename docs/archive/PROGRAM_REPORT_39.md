...
1. **Run `/scrub` for a full identity/AI-residue pass.** This dispatch's machine-path scrub was intentionally narrow (the two named globs: `docs/PROGRAM_REPORT_*.md`, `eval/gold/*.json`), per Decisions. It found 3 additional files with the identical `/Users/owieschon` residue pattern, outside its owned scope:
   - `docs/PROGRAM_REPORT.md:4` — `` Worktree: `/Users/owieschon/dev/ultra-csm-mega` ``
   - `docs/EXECUTOR_HANDOFF_LANE_I.md:33` — `` `/Users/owieschon/ultra-csm-corpus-a-PRIVATE.md` (outside the repo); committed ``
   - `docs/LIVE_INTEGRATION_FINDINGS.md:129` — `` `/Users/owieschon/ultra-csm-corpus-a-PRIVATE.md` for org identity, never ``
