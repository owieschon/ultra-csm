...
Worktree: private execution worktree (not part of the reviewer artifact)
...
| K4 nested collections | **CLOSED post-merge.** `tests/test_external_book.py` extracts nested CRMContact records via parent account identity and preserves unrepresentable paths otherwise. The previously-noted gap (extraction not recursing through an intermediate wrapper object) is fixed: schema derivation now matches a collection by its trailing field name rather than the full dotted path, and runtime child-record extraction resolves the collection's dotted path through nested dicts instead of a flat single-level lookup. Proven both synthetically (new wrapper-nested unit test) and live against corpus A (200-row bounded fetch): `data.contacts` no longer unrepresentable, real `data.contacts[].*` candidates surfaced with true 3-4/200 coverage, confirmed extraction produced CRMContact 4/200 real records with `join_coverage: {contact_candidates: 4, contacts_joined: 4, ratio: 1.0}` — genuine this time, zero fabrication. Full account in `docs/FOREIGN_CORPUS_FINDINGS.md`. |
| Relay battery | `PYTHONPATH=src:. python3 -m eval.relay_battery` -> `hard_ok=true`, score 11/11; full suite confirms byte determinism. |
| Private corpus reprobe | Runtime-only bounded read: discovery fetched 200 rows, 13 confirmations, 4 unknown, 0 silent guesses. Confirmed pass typed CRMAccount 200/200, injection markers 0. **CORRECTED post-merge**: the original CRMContact 200/200, CRMOpportunity 200/200, and "contact joins 200/200" numbers were hollow — the mechanically generated confirmation template mapped contact/opportunity identity to the same source path already used for the account, so every "contact"/"opportunity" was the account record relabeled, not a real join or a real extraction. A live host-driven re-run (reading the proposal's own candidate evidence rather than accepting the template) caught this and used `not_mappable` for those fields instead, producing the honest result: CRMContact 0/60, CRMOpportunity 0/60, zero fabricated records, `join_coverage: {contact_candidates: 0, contacts_joined: 0, ratio: null}`, and 60/60 accounts returning a loud `MISSING_CS_DATA` score error rather than a silent or fabricated score. `join_coverage.ratio` no longer defaults to `1.0` at zero candidates (fixed in `external_book.py`) — a vacuous 0/0 must never render as 100% success. Full account of the correction, including the precise K4 gap (nested collection behind a `data`-style wrapper), in `docs/FOREIGN_CORPUS_FINDINGS.md`. |
