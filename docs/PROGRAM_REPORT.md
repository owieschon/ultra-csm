...
| K4 nested collections | `tests/test_external_book.py` extracts nested CRMContact records via parent account identity and preserves unrepresentable paths otherwise. Note: extraction handles a collection nested directly on the record; it does not yet recurse through an intermediate wrapper object (e.g. a JSONB `data` envelope) to reach a collection one level deeper — found live against corpus A, see the report below. |
