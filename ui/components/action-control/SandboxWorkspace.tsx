        ...
        <section className={styles.hero}>
          ...
          <button
            ...
            title={
              sandboxApiAvailable
                ? undefined
                : "needs the sandbox API — absent in this export"
            }
            onClick={() => void reset()}
          >
            {sandboxApiAvailable
              ? "Reset with a new run"
              : "Runs need the sandbox API"}
          </button>
        </section>

        {/* Backend-absent WITH the frozen proof loaded is the hosted demo's
            intended state, not a failure — compose it as information. The
            red alert stays for real errors. */}
        {error &&
          (error.code === "SANDBOX_BACKEND_UNAVAILABLE" && frozenProof ? (
            <div className={styles.noticeInfo} role="note">
              <b>static export</b>
              <span>
                The interactive lab runs against the sandbox-only API. This
                hosted page shows the frozen proof below — generated by the
                real gate, never pretending the controls are live.
              </span>
            </div>
          ) : (
