"""Northbend Haulage -- routine health check-in, day-offset aware.

Northbend is a high-touch, steady_state account with a single scored
...
from ultra_csm.data_plane.narrative_content.northbend_content import BODIES as _BODIES
from ultra_csm.data_plane.narrative_shared import derive_snippet, rfc3339 as _rfc3339

NORTHBEND_ACCOUNT_ID = account_id_for("northbend-haulage")
_CSM_EMAIL = "csm101@fleetops-platform.example"
_JORDAN_EMAIL = "jordan.cole@northbend-haulage.example"
JORDAN_CONTACT_ID = det_id("contact", NORTHBEND_ACCOUNT_ID, _JORDAN_EMAIL)

# (day_offset, hour, from_contact, subject)
_MESSAGE_SCHEDULE: tuple[tuple[int, int, bool, str], ...] = (
    (58, 9, False, "Checking in on Northbend's health score"),
    (59, 11, True, "Re: Checking in on Northbend's health score"),
)


def northbend_email_thread(as_of_day: int) -> dict:
    """Gmail ``users.threads.get`` shape, one thread, truncated to *as_of_day*."""

    thread_id = det_id("email-thread", NORTHBEND_ACCOUNT_ID, "jordan")
    messages = []
    for day_offset, hour, from_contact, subject in _MESSAGE_SCHEDULE:
        if day_offset > as_of_day:
            break
        sender = _JORDAN_EMAIL if from_contact else _CSM_EMAIL
        recipient = _CSM_EMAIL if from_contact else _JORDAN_EMAIL
        msg_id = det_id("email-msg", NORTHBEND_ACCOUNT_ID, day_offset, hour)
...
def northbend_communication_signals(
    ...
    threads = threads if threads is not None else northbend_email_thread(as_of_day)["threads"]
    for thread in threads:
        prev_outbound_at = None
        for msg in thread["messages"]:
            headers = {h["name"]: h["value"] for h in msg["payload"]["headers"]}
            sent_at = datetime.fromisoformat(headers["Date"].replace("Z", "+00:00"))
            from_contact = headers["From"] == _JORDAN_EMAIL
            if not from_contact:
                prev_outbound_at = sent_at
                signals.append(
                    CommunicationSignal(
                        signal_id=det_id("comm-signal", NORTHBEND_ACCOUNT_ID, msg["id"]),
                        account_id=NORTHBEND_ACCOUNT_ID,
                        contact_id=JORDAN_CONTACT_ID,
                        channel="email",
                        direction="outbound",
