...
--    consented contact in the proposal payload. REST/MCP already check this at
--    the edge; this trigger makes the invariant fail-closed in the database.
--
...
BEGIN
  ...
  BEGIN
    v_contact_id := (v_payload ->> 'contact_id')::uuid;
  EXCEPTION WHEN invalid_text_representation THEN
    RAISE EXCEPTION
      'outreach consent: proposal % has non-uuid contact_id %',
      NEW.proposal_id, v_payload ->> 'contact_id'
      USING errcode = 'check_violation';
  END;

  IF v_payload ? 'account_id' THEN
    BEGIN
      v_account_id := (v_payload ->> 'account_id')::uuid;
    EXCEPTION WHEN invalid_text_representation THEN
      RAISE EXCEPTION
        'outreach consent: proposal % has non-uuid account_id %',
        NEW.proposal_id, v_payload ->> 'account_id'
        USING errcode = 'check_violation';
    END;
  END IF;
  ...
  SELECT EXISTS (
    ...
    WHERE c.tenant_id = NEW.tenant_id
      ...
      AND (v_account_id IS NULL OR c.account_id = v_account_id)
      AND c.consent IS TRUE
  ) INTO v_ok;

  IF NOT v_ok THEN
    RAISE EXCEPTION
      'outreach consent: contact % is not consented for proposal %',
      v_contact_id, NEW.proposal_id
