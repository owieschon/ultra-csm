...
ui-e2e:
	cd ui && npm ci && npm run test:e2e

action-control-contract:
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_contract.py

action-control-contract-check:
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_contract.py --check

action-control-sandbox-contract:
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_sandbox_contract.py

action-control-sandbox-check:
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_sandbox_contract.py --check
	PYTHONPATH=src:. $(PYTHON) -m pytest tests/test_action_control_sandbox.py tests/test_action_control_sandbox_api.py -q

hosted-readonly-demo:
	ULTRA_CSM_BIND_HOST=127.0.0.1 ULTRA_CSM_DEMO_NOAUTH=1 PYTHONPATH=src:. $(PYTHON) scripts/export_hosted_readonly_demo.py --check
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_contract.py --check
	PYTHONPATH=src:. $(PYTHON) scripts/export_action_control_sandbox_contract.py --check
	cd ui && npm ci && NEXT_PUBLIC_UCSM_READONLY_DEMO=1 npm run lint && NEXT_PUBLIC_UCSM_READONLY_DEMO=1 npm run build

hosted-readonly-demo-check:
	ULTRA_CSM_BIND_HOST=127.0.0.1 ULTRA_CSM_DEMO_NOAUTH=1 PYTHONPATH=src:. $(PYTHON) scripts/export_hosted_readonly_demo.py --check

hosted-readonly-demo-generate:
	ULTRA_CSM_BIND_HOST=127.0.0.1 ULTRA_CSM_DEMO_NOAUTH=1 PYTHONPATH=src:. $(PYTHON) scripts/export_hosted_readonly_demo.py

hosted-action-control-bundle:
	PYTHONPATH=src:. $(PYTHON) scripts/build_hosted_action_control_bundle.py

hosted-action-control-bundle-check:
	PYTHONPATH=src:. $(PYTHON) scripts/build_hosted_action_control_bundle.py --check

hosted-action-control-deploy-check: hosted-action-control-bundle-check
	PYTHONPATH=src:. $(PYTHON) -m pytest tests/test_hosted_action_control_bundle.py tests/test_action_control_sandbox.py tests/test_action_control_sandbox_api.py -q

eval:
	$(PYTHON) -m pytest tests/ -q
	PYTHONPATH=src:. $(PYTHON) -m eval.gold_slot_b_quality --check
	PYTHONPATH=src:. $(PYTHON) -m eval.gold_slot_b_hard --check
	PYTHONPATH=src:. $(PYTHON) -m eval.knowability_audit --check
...
self-serve-workflow-eval:
	PYTHONPATH=src:. $(PYTHON) -m eval.self_serve_workflow_eval

world:
	PYTHONPATH=src:. $(PYTHON) -m ultra_csm.cli world --seed $${SEED:-7} --scale $${SCALE:-60}

world-scoreboard:
	PYTHONPATH=src:. $(PYTHON) -m eval.world_scoreboard --seed $${SEED:-7} --scale $${SCALE:-60}
